...
def create_mitra_coords(mitra_coords: schemas.MitraCoords, db: Session = Depends(get_db), payload=Depends(jwt_auth_wrapper)):
    try:
        check_mitra_coords = db.query(models.MitraCoords).filter(
            models.MitraCoords.id_mitra == mitra_coords.id_mitra,
            models.MitraCoords.status == 0
        ).first()
        if check_mitra_coords:
            for key, value in mitra_coords.model_dump().items():
                setattr(check_mitra_coords, key, value)
            db.commit()
            return "Successfully updated mitra coordinates"
        else:
            new_mitra_coords = models.MitraCoords(**mitra_coords.model_dump(), created_on=datetime.now())
...
def get_mitra_coords(db: Session = Depends(get_db)):
    mitra_coords = db.query(models.MitraCoords).all()
    if mitra_coords:
        return mitra_coords
    else:
...
def get_mitra_coords_by_location(country_code: str, region: str, db: Session = Depends(get_db)):
    mitra_coords = db.query(models.MitraCoords).filter(
        models.MitraCoords.country_code == country_code,
        models.MitraCoords.region == region
    ).all()
    if not mitra_coords:
...
def get_mitra_coords_by_pickup(request: schemas.MitraCoordsOut, db: Session = Depends(get_db), payload=Depends(jwt_auth_wrapper)):
    try:
        cursor = db.execute(
            f"""SELECT DC.*, 
...
def delete_mitra_coords(mitra_id: int, db: Session = Depends(get_db), payload=Depends(jwt_auth_wrapper)):
    db_mitra_coords = db.query(models.MitraCoords).filter(models.MitraCoords.id_mitra == mitra_id).first()
    if not db_mitra_coords:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Mitra coordinates not found")
    
    db.delete(db_mitra_coords)
    db.commit()
